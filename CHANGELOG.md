...
### Added
- **Panel terminal: `ls`, `cd`, `nano` for panel users**: user terminal sessions now use `/bin/bash` instead of `rbash` (which disabled the `cd` builtin entirely). `~/bin/ls` and `~/bin/nano` symlinks are provisioned on user creation and PHP version changes. A root-owned `cd()` shell function in `.bash_profile`/`.bashrc` restricts navigation to within `$HOME` — attempts to `cd` outside the home directory are rejected and the working directory reverts.
