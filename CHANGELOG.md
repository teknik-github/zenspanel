...
### Added
- `docs/api/user.md` — complete user-facing API reference covering all user endpoints: auth+2FA, profile/usage, domains+suspend+backup, subdomains, SSL (Let's Encrypt + custom) for domains and subdomains, redirects, hotlink protection, domain logs, databases+password-reset+phpMyAdmin SSO, FTP accounts, backups (async + download + restore), file manager (list/read/write/mkdir/rename/delete/upload/chmod/copy/compress/extract), cron jobs, PHP extensions toggle, packages/PHP-versions read-only views, terminal WebSocket token, website installer, and antivirus scan/alerts/watch.
