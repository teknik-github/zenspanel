...
### Fixed
- **Backup download "Request failed"**: two root causes fixed. (1) Go backend's `c.FileAttachment` delegates to `http.ServeFile` which returns an HTML 404 page (not JSON) when the archive is missing from disk — added `os.Stat` check that returns `{"error":"backup file not found on disk"}` before calling `c.FileAttachment`. (2) Nitro proxy returned `ArrayBuffer` directly from the event handler; h3 JSON-serialises ArrayBuffer as `{}` so `error.data.error` was always undefined in the browser — error responses are now decoded to JSON, and the success path uses `send(event, new Uint8Array(...))` so raw binary bytes are transmitted correctly.
